...
int ViewGL::findPixelFormatWithAttributes(int colorBits, int depthBits, int stencilBits, int msaaSamples)
{
    ...
    XGL::log("Creating a dummy window to use WGL_ARB_pixel_format extension.");

	//XGL::Window tmpWin(::GetModuleHandle(0), L"tmp", 0, &tmpCtrl);
 //   tmpWin.setClassStyle(CS_OWNDC);
    ...
    if(glewIsExtensionSupported("WGL_ARB_pixel_format") && glewIsExtensionSupported("WGL_ARB_multisample"))
    {
		XGL::log("WGL_ARB_pixel_format and WGL_ARB_multisample are supported.");
		XGL::log("Use wglChoosePixelFormatARB() to find a pixel format with MSAA.");
		...
		XGL::log("MSAA mode: %dx", samples);
    }
    else
    {
		XGL::log("[WARNING] WGL_ARB_pixel_format and WGL_ARB_multisample are NOT supported.");
		XGL::log("[WARNING] MSAA cannot be enabled.");
    }
	XGL::log("Pixel Format: %d", pixelFormat);
    ...
    ::DestroyWindow(tmpHandle);
	XGL::log("A dummy window is destroyed.");

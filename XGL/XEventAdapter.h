...
namespace XGL {
	...
	struct PointerData 	
	{
		...
		PointerData( float in_x, float in_xMin, float in_xMax, float in_y, float in_yMin, float in_yMax) :
			x(in_x),
			xMin(in_xMin),
			xMax(in_xMax),
			y(in_y),
	...
	class XGL_API XEventAdapter :public XEvent
	{
	public:

		typedef std::vector< PointerData* > PointerDataList;
	...
	public:
		XEventAdapter();
		XEventAdapter(const XEventAdapter& event);
		XEventAdapter& operator=(const XEventAdapter& event);
		virtual ~XEventAdapter();
	public:
		...
		void setWindowRect(int rect[4]) {
			_windowX = rect[0];
			_windowY = rect[1];
			_windowWidth = rect[2];
			_windowHeight = rect[3];
		}

		void getWindowRect(int rect[4])const {
			rect[0] = _windowX;
			rect[1] = _windowY;

			rect[2] = _windowWidth;
			rect[3] = _windowHeight;

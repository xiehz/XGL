...
namespace XGL
{
	...
	//{
	enum XGL_API AnimationMode  // enum
	{
		LINEAR = 0,
		EASE_IN,
		EASE_OUT,
...
XGL_API int getFrame(int frameStart, int frameEnd, float elapsedTime, float frameRate=30, bool loop=true);



// spherical linear interpolation between 3D vectors
// alpha should be 0 ~ 1
XGL_API Vector3 slerp(const Vector3& from, const Vector3& to, float alpha, AnimationMode mode=LINEAR);



// spherical linear interpolation between 2 quaternions
// the alpha value should be 0 ~ 1
XGL_API Quaternion slerp(const Quaternion& from, const Quaternion& to, float alpha, AnimationMode mode=LINEAR);
...
XGL_API float accelerate(bool isMoving, float currSpeed, float maxSpeed, float accel, float deltaTime);
...
template <class T>
XGL_API bool move(T& vec, const T& from, const T& to, float elapsedTime, float speed);
...
template <class T>
XGL_API  T lerp(const T& from, const T& to, float alpha);
...
template <class T>
 XGL_API T interpolate(const T& from, const T& to, float alpha, AnimationMode mode);

 template XGL_API bool move(Vector3& vec, const Vector3& from, const Vector3& to, float elapsedTime, float speed);

 template XGL_API  Vector3 lerp(const Vector3& from, const Vector3& to, float alpha);

 template XGL_API Vector3 interpolate(const Vector3& from, const Vector3& to, float alpha, AnimationMode mode);
